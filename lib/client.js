...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LifecycleCard(props) {
      ...
      // 高级区折叠（渐进披露）：低频兜底配置默认收起，常用 6 项保持可见
      const [advOpen, setAdvOpen] = useState(false);
      useEffect(() => scope.subscribe(() => setSnap(scope.getSnapshot())), [scope]);
      // 状态快照轮询（host /status 路由，轻量 stat + 扫描缓存，30s 一次）：
      // 归档目录现状 + 最近一轮清理，只读展示
      useEffect(() => {
        let alive = true;
        const pull = () => {
          try {
            fetch('/plugins/dsh-session-pruner/status', { cache: 'no-store' })
              .then((r) => (r.ok ? r.json() : null))
              .then((s) => { if (alive && s && typeof s === 'object') setStatus(s); })
              .catch(() => { /* 路由不可用（host 旧版）则状态行不显示 */ });
          } catch (e) { /* fetch 不可用环境忽略 */ }
        };
        pull();
      ...
      const draftFromValue = (v) => ({
        ...
        pinnedIds: (v.pinnedIds ?? []).join('\n'),
      });
      // pinnedIds 规范化：每行一个，trim + 去空行（保存与 dirty 比较共用）
      const pinnedList = (t) => String(t ?? '').split('\n').map((s) => s.trim()).filter(Boolean);
      useEffect(() => {
      ...
      // dirty 判定：数字/布尔/枚举/列表各一类原子判定，总 dirty = 高级区 4 项 ∪ 其余
      // （高级区 badge 与总 dirty 共用原子判定，不重复展开）
      const numDirty = (f) => Number(draft[f]) !== (value[f] ?? DEF[f]);
      const modeDirty = (draft.archiveMode || 'archive') !== (value.archiveMode === 'delete' ? 'delete' : 'archive');
      const pinnedDirty = pinnedList(draft.pinnedIds).join('\n') !== (value.pinnedIds ?? []).join('\n');
      const advDirty = numDirty('intervalMinutes') || numDirty('maxSessions') ||
        numDirty('uiRefreshSeconds') || !!draft.cleanMain !== !!value.cleanMain;
      const dirty = advDirty || numDirty('archiveHours') || numDirty('continuableIdleDays') ||
        numDirty('mainIdleDays') || numDirty('oneShotMinAgeMinutes') || modeDirty || pinnedDirty;
      ...
      const save = async () => {
        ...
        try {
          ...
          await scope.set('pinnedIds', pinnedList(draft.pinnedIds));
        } catch (e) {
      ...
      // 状态行文案：归档现状 + 总量/超限 + 最近清理（host /status 路由，30s 轮询）
      const statusLine = (() => {
        if (!status) return null;
        const parts = [];
        if (status.archiveMode === 'archive') {
          const exp = status.archived && status.archived.oldestExpiresAt
            ? '，最早 ' + new Date(status.archived.oldestExpiresAt).toLocaleTimeString() + ' 到期'
            : '';
          parts.push('归档 ' + ((status.archived && status.archived.count) || 0) + ' 个' + exp);
        }
        if (typeof status.total === 'number' && status.total > 0) {
          const over = status.overCapacity > 0 ? '（超限 +' + status.overCapacity + '）' : '';
          parts.push('总量 ' + status.total + over);
        }
        if (status.lastScanAt) {
          parts.push('最近清理 ' + (status.lastScanRemoved || 0) + ' 个（' + new Date(status.lastScanAt).toLocaleTimeString() + '）');
        }
        if (status.pinned) parts.push('已固定 ' + status.pinned + ' 个');
        if (!parts.length) return null;
        return h('p', { style: { color: V.text3, margin: '12px 0 0', fontSize: 12, lineHeight: 1.5 } }, '状态：' + parts.join(' · '));
      })();
      ...
      const body = h('div', { style: { borderTop: '1px solid ' + V.border, margin: '0 16px', paddingBottom: 8 } },
        ...
        h('div', { style: { flexDirection: 'column', gap: 6, padding: '12px 0', display: 'flex' } },
          h('label', { htmlFor: 'slc-pinnedIds', style: { color: V.text1, fontSize: 13, fontWeight: 500, lineHeight: 1.5 } }, 'Pin 白名单（每行一个会话 ID）'),
          h('textarea', {
            id: 'slc-pinnedIds',
            value: draft.pinnedIds,
            disabled: !writable,
            rows: 3,
            placeholder: 'session-xxxxxxxx\nsession-yyyyyyyy',
            onChange: (e) => setDraft({ ...draft, pinnedIds: e.target.value }),
            style: {
              border: '1px solid ' + V.border, background: V.bgCard, font: 'inherit', color: V.text1,
              borderRadius: 8, padding: '6px 12px', fontSize: 13, lineHeight: 1.5,
              resize: 'vertical', minHeight: 60, outline: 'none',
            },
          }),
          h('p', { style: { color: V.text3, margin: 0, fontSize: 12, lineHeight: 1.5 } }, '名单内的会话永不自动清理。恢复归档会话后建议立即 pin，避免在打开前的窗口期被再次归档')),
        // ── 高级区（低频兜底配置，默认折叠）──
        h('button', {
          type: 'button',
          ...
          style: {
            appearance: 'none', width: '100%', font: 'inherit', textAlign: 'left',
            cursor: 'pointer', background: '0 0', border: 0, color: V.text3,
            alignItems: 'center', gap: 8, padding: '12px 0 4px', display: 'flex',
            borderTop: '1px solid ' + V.border, fontSize: 12, fontWeight: 500, lineHeight: 1.5,
          },
        },
          h('span', { style: { flex: 1, minWidth: 0 } }, '高级设置（对账与兜底）'),
          advDirty && !advOpen
            ? h('span', { style: { whiteSpace: 'nowrap', background: V.bgModule, color: V.text2, borderRadius: 999, padding: '1px 8px', fontSize: 11, fontWeight: 500, lineHeight: '17px' } }, '有未保存更改')
            : null,
          h('span', { style: { flex: 'none', transform: advOpen ? 'rotate(180deg)' : 'none', transition: 'transform .16s', fontSize: 10 } }, '▾')),
        advOpen ? h('div', null,
          field('intervalMinutes', '扫描间隔（分钟）', '定时对账扫描周期（事件驱动为主路径，此值为兜底）'),
          field('maxSessions', '容量保底（会话数）', '超限按 one-shot → 可续 → 主会话 回收最旧'),
          field('uiRefreshSeconds', '界面兜底刷新间隔（秒）', 'dirty-flag 为主路径（3s 变更检测），此值为全量刷新兜底'),
          h('div', { style: { flexDirection: 'column', gap: 6, padding: '12px 0', display: 'flex' } },
            h('div', { style: { alignItems: 'center', gap: 8, display: 'flex' } },
