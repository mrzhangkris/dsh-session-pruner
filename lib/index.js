...
const INTERVAL_MS =
  Number(process.env.DSH_SESSION_LIFECYCLE_INTERVAL_MS) || 6 * 3600 * 1000
const MAX_SESSIONS = Number(process.env.DSH_SESSION_LIFECYCLE_MAX) || 400
const CLEAN_MAIN = process.env.DSH_SESSION_LIFECYCLE_CLEAN_MAIN === '1'
...
async function runOnce(ctx) {
  ...
  // 2) 容量保底
  if (keep.length > MAX_SESSIONS) {
    const excess = keep.length - MAX_SESSIONS
    ...
    for (const s of sorted) {
      ...
      if (s.origin === 'main' && !CLEAN_MAIN) continue
...
export function apply(ctx) {
  ...
  ctx.timer.setInterval(safeRun, INTERVAL_MS)
  console.log(
    `[session-lifecycle] armed: interval=${INTERVAL_MS / 3600000}h ` +
      `cap=${MAX_SESSIONS} cleanMain=${CLEAN_MAIN}`,
  )
}
