...
export const Config = z.object({
  ...
  /** 归档保留小时数：清理的会话先归档（移出 sessions 目录），N 小时后物理删除；0 = 立即删除不归档 */
  archiveHours: z.number().min(0).max(720).default(24),
})
...
async function pruneArchive(ctx) {
  if (runtime.archiveHours <= 0) return
