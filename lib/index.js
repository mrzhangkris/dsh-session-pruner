...
/** 删除会话目录 + 连带清除 projcache 缓存行 + workspace 记账。返回是否清理了缓存。 */
async function removeSession(ctx, entry, reason) {
  ...
  try {
    ...
    console.log(
      `[session-lifecycle] purge debug: domain=${domain ? 'yes' : 'NO'} ` +
        `table=${table ? 'yes' : 'NO'} delete=${typeof table?.delete === 'function' ? 'yes' : 'NO'}`,
    )
    if (table && typeof table.delete === 'function') {
      const before = table.size ?? -1
      const result = await table.delete(entry.sid)
      console.log(
        `[session-lifecycle] purge debug: result=${result} before=${before} after=${table.size ?? -1}`,
      )
      cache = true
    }
  } catch (e) {
