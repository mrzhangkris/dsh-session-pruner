...
export const NS = settingsNamespace('dsh-session-pruner')
...
async function runOnce(ctx) {
  ...
  // 1) one-shot 子代理：闲置超过 oneShotMinAgeMinutes 分钟即归档（配置统一控制，
  //    有/无 end-seed 同阈值）——结果已回传主会话，过程无价值。
  ...
  for (const s of all) {
    ...
    if (s.origin === 'subagent' && s.mode === 'one-shot') {
      ...
      // 闲置超过 oneShotMinAgeMinutes 即归档（有/无 end-seed 统一阈值，配置控制）
      const done = ageMs > runtime.oneShotMinAgeMs
      if (done) {
        await archiveSession(ctx, s, 'one-shot idle')
        removed++
        continue
      }
...
export function apply(ctx, config) {
  ...
  try {
    installSettingsSection(ctx, NS, Config, config, {
      ...
      onChange: () => {
        console.log('[dsh-session-pruner] debug: onChange fired')
        try {
          const c = source()
          ...
          console.log(
            `[dsh-session-pruner] hot-reloaded: interval=${(c.intervalMinutes ?? 60)}min ` +
