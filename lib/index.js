...
/** live 检查（fail-closed 单一来源，runOnce/容量保底/事件路径三处共用）：
 * 只有「sessions 服务可用且会话确实不在 store」才返回 false（可清理）。
 * 任何不确定路径一律视为 live——宁可不删不可误删（delete 模式误删不可恢复）：
 * - ctx.get('sessions') 返回 undefined：cordis registry.get strict 模式在 provider fiber
 *   非 active（服务重载窗口）时返回 undefined 而非抛异常——旧版 try/catch 只防了抛
 *   异常路径，此窗口整体 fail-open（审计🔴b，sessions 服务重载窗口内最坏批量 rm）
 * - sessions.get 接口缺失或查询抛异常：同样视为 live
 * @param {object} ctx host ctx（e2e mock ctx 必须提供 get: () => ({ get: () => undefined })）
 */
...
async function runOnce(ctx) {
  ...
  if (keep.length > runtime.maxSessions) {
    ...
    for (const s of sorted) {
      ...
      if (!s.ended) continue
      // live 保护（isLive fail-closed 单一来源）：内存 store 挂着的会话不清理
      if (isLive(ctx, s.sid)) continue
...
async function archiveCandidate(sid) {
  ...
  if (!ctx) return
  try {
    if (isLive(ctx, sid)) return // live 保护（fail-closed）：仍在 store 或服务不可判定时不归档
