...
function recordArchive(sessionId) {
  archiveSeq++
  archiveLog.push({ seq: archiveSeq, sessionId, at: Date.now() })
  ...
  try {
    webServer.register({
      ...
      handler: (req, res) => {
        ...
        const archived = archiveLog.filter((r) => r.seq > since).map((r) => r.sessionId)
        res.writeHead(200, {
